...
A chatbot with a FastAPI backend (`app.py`) and Streamlit frontend (`streamlit_ui.py`), hosted at [https://huggingface.co/spaces/roar3691/chatbot].
...
3. Run FastAPI backend:
   ...
   python app.py
   ```
4. In a new terminal, run Streamlit frontend:
   ```bash
...
- `streamlit_ui.py`: Streamlit frontend (port 8501 locally, 7860 on Spaces).
- `Dockerfile`: Docker config for Spaces.
...
required_packages = [
    "fastapi==0.103.1", "uvicorn==0.23.2", "pymongo==4.5.0", "python-dotenv==1.0.0",
    "aiohttp==3.8.5", "bcrypt==4.0.1", "google-api-python-client==2.100.0"
]
...
async def summarize_chat(request: SummaryRequest):
    ...
    prompt = f"Summarize this conversation and provide insights (sentiment, keywords):\n{history}"
...
required_packages = [
    "streamlit==1.27.0", "requests==2.31.0", "pymongo==4.5.0",
    "python-dotenv==1.0.0", "bcrypt==4.0.1"
]
for package in required_packages:
    install_package(package)
...
EXPOSE 7860
CMD ["bash", "-c", "python app.py & streamlit run streamlit_ui.py --server.port 7860 --server.address 0.0.0.0"]
```
